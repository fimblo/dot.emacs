...
First let's set up the archives.
...
#+begin_src emacs-lisp
  ...
  (setq scroll-conservatively 101)              ; never "jump" the cursor to center when scrolling 
  (setq scroll-margin 3)                        ; start scrolling three lines from the edge
  (setq pixel-scroll-precision-interpolation-total-time 0.2) ; lower = faster response
