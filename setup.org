...
followed by <tab> will create an org code block.
...
=setup.org= resides in =~/.emacs.d=. But anyway.

The important bit is that emacs loads =org-mode=, then removes all the
text which is meant for humans, leaving the emacs-lisp code. This code
is then evaluated at startup time resulting in a fully configured
emacs instance.

This makes it possible for me to write my emacs setup as an org file,
interleaving comments like this sentence with actual code. Since I
configure emacs infrequently, these proper comments are really useful
for me. Babel-mode is quite nifty, actually.
...
#+BEGIN_SRC emacs-lisp
  ;; sets a bunch of defaults to be consistent with UTF-8
  (set-language-environment   "UTF-8")

  ;; default to UTF-8 when creating new buffers and files
  (set-default-coding-systems 'utf-8)

  ;; When guessing a file's encoding, try UTF-8 first
  (prefer-coding-system       'utf-8)
#+END_SRC

** Auto-customization

Emacs has a bunch of point-and-click customization features.

Move these configurations to a separate file which isn't version
controlled. I rarely use them anyway.

I want to do this early, since I might want to use some of the vars
later in the code.
...
#+BEGIN_SRC emacs-lisp
  ...
  (defvar me/erc/autojoin-alist    nil
                           "Association list of channels to join.
                            For example:
                            ((\"chat.freenode.net\" \"#emacs\" \"#cooking\")
                             (\"another.server.org\" \"#foo\" \"#bar\" \"#baz\"))
                            ")
  (defvar me/erc/pass-query-string nil
                           "How should ERC ask for the password?
                            Useful if you have multiple servers to connect to.")

  (defvar me/vterm-clients
    '(("acme"    . "~/.bash_history_acme")
      ("globex"  . "~/.bash_history_globex"))
                           "Alist of vterm client sessions: (NAME . HISTFILE).
                            NAME labels the buffer and is exported as EMACS_VTERM_CLIENT.
                            HISTFILE is the bash history path.
                            Override this with setq in EMACS_PERSONAL_DATA.")
...
(setq native-comp-deferred-compilation t)               ; enable async native compilation
(setq native-comp-async-jobs-number 4)                  ; # of cpu cores to use
(setq native-comp-async-report-warnings-errors 'silent) ; silent mode when compiling
(setq native-comp-eln-load-path                         ; where to store the compiled stuff
      (list (expand-file-name "eln-cache/" user-emacs-directory)))

;; Tell the native compiler to look at the vendor dirand report when done
(setq native-comp-async-cu-done-functions
      (lambda (file) (message "Native compile finished: %s" file)))
...
(package-initialize) ; make package-installed-p and the archive cache available


;;;;;;;;; CONDITIONAL REFRESH ;;;;;;;;;

(let* ((my/archive-file (expand-file-name "elpa/archives/melpa/archive-contents" user-emacs-directory))
       (my/file-exists  (file-exists-p my/archive-file)))
  (cond
   ;; First run, force refresh
   ((not my/file-exists)
    (message "No package cache found. Initializing...")
    (package-refresh-contents))

   ;; File exists and is old (older than 7 days). refresh time!
   ((> (time-to-number-of-days
        (time-subtract (current-time)
                       (file-attribute-modification-time (file-attributes my/archive-file))))
       7)
    (progn
      (message "Package cache is older than 7 days. Refreshing...")
      (package-refresh-contents)))

   ;; File exists and is recent. no refresh
   (t
    (message "Package cache is fresh. Skipping refresh."))))
...
;; Install missing packages from the list above
...
;; Setup use-package
(require 'use-package)
(setq use-package-always-ensure t) ; makes ":ensure t" the default


;;;;;;;;; VENDOR DIRECTORY SETUP ;;;;;;;;;

;; Add 'vendor' to load-path so we can 'require' files inside it
(let ((vendor-dir (expand-file-name "vendor" user-emacs-directory)))
  ...
  ;; Add all subdirectories of vendor to load-path as well
  (let ((default-directory vendor-dir))
    (normal-top-level-add-subdirs-to-load-path)))


;;;;;;;;; LOADING & COMPILING VENDOR ;;;;;;;;;
...
(me/load-directory (expand-file-name "vendor/" user-emacs-directory))

;; Quietly byte-compile any vendor files that have changed
(let ((save-silently t))
...
#+BEGIN_SRC emacs-lisp
  ...
  (add-hook 'prog-mode-hook
            (lambda ()
              (display-line-numbers-mode 1) ; show line number in margin
...
#+BEGIN_SRC emacs-lisp
  ...
  (pixel-scroll-precision-mode 1)     ; scroll pixel by pixel instead of
                                      ; line by line
...
#+BEGIN_SRC emacs-lisp
  ...
  (advice-add 'vc-follow-link :around           ; without asking; notify instead
              (lambda (orig)
                (let ((target (file-chase-links buffer-file-name)))
                  (funcall orig)
                  (message "Followed symlink → %s" target)
...
Since emacs 29 (released in 2023), we now join all the other cool
programs of the 1990s and we now can scroll pixel by pixel. Amazing.

#+begin_src emacs-lisp
  (setq scroll-step 1)                   ; scroll 1 line at a time
  (setq scroll-conservatively 101)       ; never "jump" the cursor to center
                                         ; when scrolling
  (setq scroll-margin 3)                 ; start scrolling three lines from the edge
  (setq pixel-scroll-precision-interpolation-total-time 0.1) ; lower = faster response
  (setq pixel-scroll-precision-use-momentum t) ; continue after wheel has stopped
  (setq pixel-scroll-precision-momentum-seconds 0.1) ; continue for 0.1 seconds
#+end_src
...
The =tsdh-dark= theme handles colors and appearance.
...
As of emacs v30, it's a built-in module.

#+begin_src emacs-lisp
  (use-package which-key
    :ensure nil  ; "nil" because it is now built-in to Emacs 30
    :config
...
Enable support for creating pdf from tex files, highlighting etc.

To build and display a tex file as pdf, hit =C-c C-a=, mapped to
=TeX-command-run-all=.

This text below modifies the syntax highlighting of the content of
=\marginnote{}= so that it stands out.

#+BEGIN_SRC emacs-lisp
(defun my-latex-marginnote-font-lock ()
  (font-lock-add-keywords
   nil
   '(("\\\\marginnote{\\(\\([^}]*\\)\\)}"
...
(add-hook 'LaTeX-mode-hook #'my-latex-marginnote-font-lock)
#+END_SRC

** Bookmark mode

This mode enables me to bookmark positions in files, making jumping
between them nice and quick.

#+begin_src emacs-lisp
    (use-package bm
    :ensure t
    :bind (("C-<f2>"    . bm-toggle)     ; Use C-SPC to toggle a fringe bookmark
           ("M-<f2>"    . bm-next)       ; Jump between fringe marks
           ("M-S-<f2>"  . bm-previous))
    :init
    ;;  This puts the marker in the fringe instead of highlighting the line
...
#+BEGIN_SRC emacs-lisp
  ...
  (with-eval-after-load 'flycheck
    ;; Disable checkers that are either annoying (checkdoc) or currently buggy (org-lint)
    (setq-default flycheck-disabled-checkers '(emacs-lisp-checkdoc org-lint)))
#+END_SRC
...
On mac, install aspell using =brew install aspell=. On debian, =apt install aspell=
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun me/ibuffer-set-filter-groups ()
    "Group project buffers via projectile, with all terminal buffers (vterm,
     ansi-term/term, shell, eshell) pulled into their own \"Terminals\"
     group first."
    (ibuffer-projectile-set-filter-groups)
    (setq ibuffer-filter-groups
          (cons '("Terminals" (or (mode . vterm-mode)
                                  (mode . term-mode)
                                  (mode . shell-mode)
                                  (mode . eshell-mode)))
  ...
  (add-hook 'ibuffer-hook
            (lambda ()
              (hl-line-mode 1)             ; highlight current line, same as prog-modes
              (me/ibuffer-set-filter-groups)
...
I used to keep a bullet-journal for some years.

One thing I really liked about the bullet-journal system was the way
it helped me organise and consolidate old incomplete tasks.


Every so often, I would create a section under today's date called
"Migrated".

Then, if there was a task from a previous page in the notebook which
was still important, I would copy it to the Migrated section and mark
the old todo box with a =>= symbol.

Stuff which wasn't important I would mark with a hyphen =-=.

The code below emulates this, by cycling through the following four
states instead of just the default two:

- =[ ]= to-do
- =[X]= done
- =[-]= skipped / irrelevant
- =[>]= migrated to a future date

Pressing =C-c C-c= on a checkbox item cycles to the next state.

#+BEGIN_SRC emacs-lisp
  (defun me/org-at-any-checkbox-p ()
    "Return t if point is at a list item with any checkbox, including [>]."
    (save-excursion
      (beginning-of-line)
      (looking-at "[ \t]*[-+*][ \t]+\\[[X> -]\\]")))

  (defun me/org-checkbox-cycle ()
    "Cycle checkbox through [ ], [X], [-], [>] states."
    (interactive)
    (save-excursion
      (beginning-of-line)
      (when (re-search-forward "\\[\\([X> -]\\)\\]" (line-end-position) t)
        (let* ((current (match-string 1))
               (next (cdr (assoc current '((" " . "X") ("X" . "-") ("-" . ">") (">" . " "))))))
          (when next
            (replace-match next nil nil nil 1)))))
    (me/org-update-parent-stats))

  (defun me/org-update-parent-stats ()
    "Update parent [n/m] or [n%] cookie when cycling a child checkbox.
  [X] counts toward both done and total; [ ] toward total only;
  [-] (skip) and [>] (defer) are excluded from both."
    (save-excursion
      (beginning-of-line)
      (let ((child-indent (current-indentation)))
        (when (> child-indent 0)
          (let ((parent-bol nil))
            (save-excursion
              (while (and (> (point) (point-min)) (null parent-bol))
                (forward-line -1)
                (unless (looking-at "^[ \t]*$")
                  (if (< (current-indentation) child-indent)
                      (setq parent-bol (point))))))
            (when (and parent-bol
                       (save-excursion
                         (goto-char parent-bol)
                         (looking-at "[ \t]*[-+*][ \t]")))
              (save-excursion
                (goto-char parent-bol)
                (when (re-search-forward "\\[\\([0-9]*/[0-9]*\\|[0-9]*%\\)\\]"
                                         (line-end-position) t)
                  (let* ((cookie-beg (match-beginning 0))
                         (cookie-end (match-end 0))
                         (is-percent (string-match "%" (match-string 1)))
                         (done 0) (total 0))
                    (forward-line 1)
                    (while (not (eobp))
                      (cond
                       ((looking-at "^[ \t]*$") (forward-line 1))
                       ((< (current-indentation) child-indent) (goto-char (point-max)))
                       ((= (current-indentation) child-indent)
                        (cond
                         ;; done: counts toward both numerator and denominator
                         ((looking-at "[ \t]*[-+*][ \t]+\\[X\\]")
                          (setq done (1+ done))
                          (setq total (1+ total)))
                         ;; skip [-] and defer [>]: removed from consideration
                         ((looking-at "[ \t]*[-+*][ \t]+\\[[->]\\]"))
                         ;; todo [ ]: counts toward denominator only
                         ((looking-at "[ \t]*[-+*][ \t]+\\[ \\]")
                          (setq total (1+ total))))
                        (forward-line 1))
                       (t (forward-line 1))))
                    (goto-char cookie-beg)
                    (delete-region (point) cookie-end)
                    (insert (if is-percent
                                (format "[%d%%]" (if (zerop total) 0
                                                   (/ (* 100 done) total)))
                              (format "[%d/%d]" done total))))))))))))

  (defun me/org-ctrl-c-ctrl-c ()
    "Like `org-ctrl-c-ctrl-c' but uses extended checkbox cycling."
    (interactive)
    (if (me/org-at-any-checkbox-p)
        (me/org-checkbox-cycle)
      (org-ctrl-c-ctrl-c)))

  (with-eval-after-load 'org
    (define-key org-mode-map (kbd "C-c C-c") #'me/org-ctrl-c-ctrl-c))
#+END_SRC

=C-c C-x >= helps me migrate tasks in org-mode. It copies the checkbox
item at point (and any children) to the top of the =Migrated Todos=
section, then stamps the original as =[>]=. If there's no such section
yet, it'll ask whether to create one at the very top of the file.

#+BEGIN_SRC emacs-lisp
  (defun me/org-item-and-children ()
    "Return text of the list item at point, including any child items."
    (save-excursion
      (beginning-of-line)
      (unless (looking-at "\\([ \t]*\\)[-+*][ \t]")
        (user-error "Not at a list item"))
      (let ((beg (point))
            (bullet-col (length (match-string 1))))
        (forward-line 1)
        (while (and (not (eobp))
                    (or (looking-at "^[ \t]*$")
                        (> (current-indentation) bullet-col)))
          (forward-line 1))
        (buffer-substring-no-properties beg (point)))))

  (defun me/org-migrate-todo ()
    "Copy current checkbox item and children to the Migrated Todos section.
  Marks the original item as [>] (migrated)."
    (interactive)
    (unless (me/org-at-any-checkbox-p)
      (user-error "Not on a checkbox item"))
    (let* ((item-text (me/org-item-and-children))
           (section-pos (save-excursion
                          (let ((case-fold-search t))
                            (goto-char (point-min))
                            (when (re-search-forward "Migrated Todos" nil t)
                              (line-beginning-position))))))
      (when (null section-pos)
        (unless (yes-or-no-p "No 'Migrated Todos' section found. Create at top of file? ")
          (user-error "Aborted"))
        (save-excursion
          (goto-char (point-min))
          (insert "Migrated Todos\n\n"))
        (setq section-pos 1))
      (save-excursion
  ...
  (with-eval-after-load 'org
    (define-key org-mode-map (kbd "C-c C-x >") #'me/org-migrate-todo))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun me/insert-date-weekday ()
    "Insert date and weekday at point in format %Y/%m/%d Weekday."
    (interactive)
...
I've started using vterm for terminals inside emacs. Since I work with
different clients, I want separate bash histories for each one.

The client list lives in =EMACS_PERSONAL_DATA= (see
[[#declare-personal-variables][Declare personal variables]]) as
=me/vterm-clients=, an alist of =(name . histfile)= pairs.

=M-x me/vterm-open= prompts for a client name and opens a fresh
terminal with =HISTFILE= set accordingly. Picking =default= just opens
a plain vterm. It starts in =~= by default; with a prefix argument
(=C-u=), it starts in the current buffer's directory instead.

#+BEGIN_SRC emacs-lisp
  (defun me/vterm-open (client &optional use-cwd)
    "Open a new vterm session for CLIENT with a dedicated bash history file.
  Prompts with completion over `me/vterm-clients' plus \"default\".
  Always opens a fresh buffer; existing sessions are left untouched.
  Client buffers are named *vterm-<client>*, *vterm-<client><2>*, etc.
  HISTFILE is set to the path specified in `me/vterm-clients'.
  Starts in the home directory, unless USE-CWD (the prefix argument)
  is non-nil, in which case it starts in the current buffer's directory."
    (interactive
     (list (completing-read "Client: "
                            (cons "default" (mapcar #'car me/vterm-clients))
                            nil t)
           current-prefix-arg))
    (let ((default-directory (if use-cwd default-directory (expand-file-name "~"))))
      (if (string= client "default")
          (vterm (generate-new-buffer-name vterm-buffer-name))
        (let* ((entry    (assoc client me/vterm-clients))
               (histfile (expand-file-name (cdr entry)))
               (buf-name (generate-new-buffer-name (concat "*vterm-" client "*")))
               (vterm-environment (cons (concat "HISTFILE=" histfile)
                                        (cons (concat "EMACS_VTERM_CLIENT=" client)
                                              (bound-and-true-p vterm-environment)))))
          (vterm buf-name)))))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (global-set-key [ f7 ]                'me/vterm-open)
  ...
  (add-hook 'prog-mode-hook
            (lambda ()
              (progn
                ...
                (local-set-key [ M-f8 ]        'display-line-numbers-mode)
