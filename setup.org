...
;; If running with org-babel, the variable 'me' does not contain the
;; file filename.org; the file gets tangled into filename.el
(setq me (or load-file-name (buffer-file-name)))
(setq me-org (replace-regexp-in-string ".el" ".org" me))

(defvar emacs-root
  (file-name-directory (or (file-symlink-p me) me)))
...
** Auto-customization
   ...
   Move all customization stuff to another file, and I won't load
   it. I don't want it cluttering my stuff, and I like all my
   configuration to be under my control.
...
(add-hook 'cperl-mode-hook 'cperl-mode-set-fat-comma)
#+END_SRC
** javascript-mode

   Since JavaScript since ES6 supports fat arrow notation for lambda
   expressions, let's add it here too.

#+BEGIN_SRC emacs-lisp
(add-hook 'javascript-mode-hook 'cperl-mode-set-fat-comma)
#+END_SRC
** Mail and Mutt mode
*** Basics
...
(defun open-dot-emacs ()
  ...
  (search-forward (concat (file-name-base me-org) (file-name-extension me-org t)))
  )
