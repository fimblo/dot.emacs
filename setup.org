...
First let's set up native machine compilation of the emacs-lisp libs.
...
Now let's set up the archives.

Since GNU and Non-GNU are built-in defaults since 29.3, we only need
to add MELPA.

I also want to have that place in the filesystem where I can put my
own .el files which will be automatically loaded. For historical
reasons, I call it 'vendor'.

#+BEGIN_SRC emacs-lisp
  (require 'package)
  (add-to-list 'package-archives '("melpa" . "https://melpa.org/packages/") t)

  ;; Setup 'vendor' directory and load paths
  ...
  ;; Tell the native compiler to look at the vendor dir
  (setq native-comp-async-cu-done-functions
        (lambda (file) (message "Native compile finished: %s" file)))

#+END_SRC

Running =(package-refresh-contents)= feels quite expensive since emacs
is (still!) single-threaded and each http call to melpa, elpa.gnu and
elpa.nongnu takes a little time.

The code below refreshes the local copy of the archives if it's more
than 7 days old or if it doesn't exist.

#+BEGIN_SRC emacs-lisp
(let* ((my/archive-file (expand-file-name "elpa/archives/melpa/archive-contents" sys/emacs-root))
  ...
  (cond
   ...
   ;; File exists and is old. refresh time!
...
I'd like to start using =use-package=, and it seems like I can have
the old (package.el) and the new (use-package) side-by-side while I
transition.

#+BEGIN_SRC emacs-lisp
  (require 'use-package)

  ;; makes ":ensure t" the default for all use-package blocks
  (setq use-package-always-ensure t)
#+END_SRC

** Packages to install

Here's the list of packages I want to have installed.

Warning: When starting emacs without the =elpa/= directory in place,
it will take some time to start up, since it needs to download
everything.

Also: This list will over time become smaller over time, since I'm
migrating to use =use-package= - and it does not require a master list
...
;; Install any package in the list that isn't already present
