...
** Cperl-mode
   ...
   Cperl-mode has more useful features than plain Perl-mode. To turn
   most of them on, set =cperl-hairy= to =t=.

   By setting =cperl-electric-parens= to =t=, I get auto-complete of
   the following paired symbols: =({[]})= and in special cases, like
   in the following code, the =<>= too.
...
#+END_SRC
   ...
   Some keywords get auto-expanded, like =if=, =while=, =for=,
   =unless=, =until= etc if you set =cperl-electric-keywords= to
   =t=.
