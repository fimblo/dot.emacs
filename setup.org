...
Refresh the local copy of the package list if it's old.
...
Also: This list will over time become smaller, since =use-package=
does not require a master list of packages to install. As I migrate
package configuration to use =use-package=, this list should become
shorter.
