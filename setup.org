...
#+BEGIN_SRC emacs-lisp
  (defvar elpa-packages
    '(
      ...
      imenu-list                          ; for showing code symbols in sidebar
      json-mode
      js2-mode
...
#+BEGIN_SRC emacs-lisp
  ...
  (add-hook 'prog-mode-hook
            (lambda ()
              ...
              (turn-on-diff-hl-mode)      ; show changes compared to vc
              )
            )
#+END_SRC
...
Fix the fringe symbols so that they are more visible.

For some reason, these don't work very well at all.
#+begin_src emacs-lisp
  (setq diff-hl-fringe-indicator-alist
      '((insert . ("+" . diff-hl-insert))
        (delete . ("-" . diff-hl-delete))
        (change . ("~" . diff-hl-change))))
  (setq diff-hl-show-hunk-numbers t)
  (setq diff-hl-draw-borders t)
  (setq diff-hl-fringe-side 'right)
#+end_src
...
#+begin_src emacs-lisp
  ...
  (defvar me/sidebar-state 0
    "Track the state of the sidebar.
     0 - No sidebar, 1 - Treemacs, 2 - imenu-list.")

  (defun me/toggle-sidebar ()
    "Toggle between no sidebar, Treemacs, and imenu-list."
    (interactive)

    ;; increment and wrap around
    (setq me/sidebar-state (mod (1+ me/sidebar-state) 3))
    ...
    (cond
     ...
     (t
      ...
      (when (get-buffer-window "*Treemacs*")
        (delete-window (get-buffer-window "*Treemacs*")))
  ...
  ;; Optional: bind this function to a key
  (global-set-key (kbd "<your-preferred-key>") 'me/toggle-sidebar)
#+end_src
...
To show code symbols in the sidebar next to treemacs
#+begin_src emacs-lisp
  (require 'imenu-list)
  ...
  ;; Keybinding to toggle the imenu-list
