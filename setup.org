...
*** Org-mode
   ...
   =C-c C-,= : (org-insert-structure-template) Insert structure block
   (like code, comment, etc). If region is selected, the structure
   will wrap around it.

   =C-c C-'= : (org-edit-special) If in a structure block, spawns a
   separate window for editing the contents of the block. If code
   structure block, will set the right major mode. Exit using the same
   =C-c C-'= key sequence.
