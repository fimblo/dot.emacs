...
#+BEGIN_SRC emacs-lisp
  ...
  (custom-set-faces
   ...
   '(ivy-minibuffer-match-face-1 ((t (:background "black" :foreground "#887733"))))
   '(ivy-minibuffer-match-face-2 ((t (:background "grey10" :foreground "#887733"))))
   '(ivy-minibuffer-match-face-3 ((t (:background "grey18" :foreground "#887733"))))
   )
...
With the above configuration, I can run shell commands in my org-mode files. All examples below prefixed with a =.= to force the interpreter of this doc to _not_ evaluate it...
...
For keybindings, see: [[Buffer manipulation]].
...
#+begin_src emacs-lisp
  (add-hook 'cperl-mode-hook
            #'(lambda () (local-set-key (kbd "M-,") 'insert-fat-comma)))
#+end_src
