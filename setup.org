...
#+END_SRC
   ...
   Finally, some hooks. The first one is a keybinding which inserts a
   fat comma on =M-,=. The second autoloads =flymake-mode= when cperl
   is loaded.
...
;(global-set-key [(shift down)]   '(lambda () (interactive) (scroll-other-window -3))
;(global-set-key [(shift up)]   '(lambda () (interactive) (scroll-other-window 3))
